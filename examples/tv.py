...
class TestExamples(BaseTest):
    ...
    def test_total_variation(self, n=1000):
        ...
        # Compute results
        self.z_true_approx, self.u_true_approx, _, _ = AA_ADMM_ZU(admm_update, D, B, b, 12, self.rho, self.maxit, self.eps_abs, self.eps_rel)
        _, _, r0, e0 = AA_ADMM_ZU(admm_update, D, B, b, 0, self.rho, self.maxit, self.eps_abs, self.eps_rel, z_true=self.z_true_approx, u_true=self.u_true_approx)
        _, _, r1, e1 = AA_ADMM_ZU(admm_update, D, B, b, 1, self.rho, self.maxit, self.eps_abs, self.eps_rel, z_true=self.z_true_approx, u_true=self.u_true_approx)
        _, _, r2, e2 = AA_ADMM_ZU(admm_update, D, B, b, 2, self.rho, self.maxit, self.eps_abs, self.eps_rel, z_true=self.z_true_approx, u_true=self.u_true_approx)
        _, _, r3, e3 = AA_ADMM_ZU(admm_update, D, B, b, 3, self.rho, self.maxit, self.eps_abs, self.eps_rel, z_true=self.z_true_approx, u_true=self.u_true_approx)
        _, _, r5, e5 = AA_ADMM_ZU(admm_update, D, B, b, 5, self.rho, self.maxit, self.eps_abs, self.eps_rel, z_true=self.z_true_approx, u_true=self.u_true_approx)
        _, _, r10, e10 = AA_ADMM_ZU(admm_update, D, B, b, 10, self.rho, self.maxit, self.eps_abs, self.eps_rel, z_true=self.z_true_approx, u_true=self.u_true_approx)
        ...
        _, _, r_sAA1, e_sAA1 = AA_ADMM_ZU(admm_update, D, B, b, 1, self.rho, self.maxit, self.eps_abs, self.eps_rel, z_true=self.z_true_approx, u_true=self.u_true_approx, use_sAA=True, beta=beta)
        _, _, r_sAA2, e_sAA2 = AA_ADMM_ZU(admm_update, D, B, b, 2, self.rho, self.maxit, self.eps_abs, self.eps_rel, z_true=self.z_true_approx, u_true=self.u_true_approx, use_sAA=True, beta=self.beta12)
