...
#                 yLabel=r'$\sqrt{||z_k-z^*||_2^2+||u_k-u^*||_2^2}$')
    ...
    def plot_aa_admm_errors(self):
        ...
        rho_ref1 = (e_sAA[0]+0.6) * np.power((1-np.sqrt(1-self.rho_M)), np.linspace(0, 400, 401))
        # Plot errors comparing ADMM and AA-ADMM
        self.plot_results(self.errors[:8]+[rho_ref1], \
                labels=['ADMM', 'AA(1)-ADMM', 'AA(2)-ADMM', 'AA(3)-ADMM', 'AA(5)-ADMM', 'AA(10)-ADMM', 'rADMM(1.9)', 'sAA(1)-ADMM', r'$\rho^*_{sAA(1)}$'], \
                linestyles=['-', '-', '-', '-', '-', '-', '-.', '-.', ':'],\
                colors=['k', 'r', 'g', 'b', 'c', 'gray', 'm', 'r', 'r'],\
                linewidths=[3.5, 3.5, 3.5, 3.5, 3.5, 3.5, 2.5, 2.5, 1.5], \
                pltError=True, \
                yLabel=r'$\sqrt{||z_k-z^*||_2^2+||u_k-u^*||_2^2}$',\
                filename="iters_lasso_0-01.png")
...
#                 yLabel=r'$\sqrt{||z_k-z^*||_2^2+||u_k-u^*||_2^2}$')
    ...
    def plot_timings(self):
        self.plot_results(self.errors[:7], ts=self.timings, \
                          ...
                          pltError=True, \
                          yLabel=r'$\sqrt{||z_k-z^*||_2^2+||u_k-u^*||_2^2}$',\
                          filename='timing_lasso_0-01.png')
...
#         plt.scatter(X, Y, marker='*', label=r"$\sigma(\Psi_2'(X^*))$")
        ...
        plt.savefig('eigs_lasso_0-01.png')
        plt.show()
