...
class TestExamples(BaseTest):
    ...
    def test_constrained_regularized_logistic_regression(self):
        ...
        # accelearte ADMM by over-relaxation ('relax' = 1.9 gives the best convergence)
...
#         _, _, _, e_rx8, t_rx8 = AA_ADMM_ZU(admm_update, A, B, b, 0, self.rho, self.maxit, self.eps_abs, self.eps_rel, relaxation=1.8, z_true=self.z_true_approx, u_true=self.u_true_approx)
        _, _, _, e_rx, t_rx = AA_ADMM_ZU(admm_update, A, B, b, 0, self.rho, self.maxit, self.eps_abs, self.eps_rel, relaxation=1.9, z_true=self.z_true_approx, u_true=self.u_true_approx)
...
#         self.relax_errors = [e_rx1, e_rx2, e_rx3, e_rx4, e_rx5, e_rx6, e_rx7, e_rx8, e_rx9]
    ...
    def plot_aa_admm_errors(self):
        ...
        e_sAA = self.errors[-1]
        rho_ref1 = e_sAA[0] * np.power((1-np.sqrt(1-self.rho_M)), np.linspace(0, 83, 84))
        ...
        self.plot_results(self.errors + [rho_ref1, rho_ref2], \
                ...
                maxIt=150,\
                filename='iters_constr_logreg.png')
    ...
    def plot_timings(self):
        self.plot_results(self.errors[:7], ts=self.timings, \
                          ...
                          pltError=True,\
                          yLabel=r'$\sqrt{||z-z^*||_2^2+||u-u^*||_2^2}$',\
                          filename='timing_constr_logreg.png')
    ...
    def plot_eigs(self):
        ...
        plt.savefig('eigs_constr_logreg.png')
        plt.show()
